...
class ChecklistTemplateTest < ActiveSupport::TestCase
  test "seeds one published alpha-1 template per machine class with shared criteria merged" do
    assert_equal 5, ChecklistTemplate.published.count
    skid = ChecklistTemplate.find_by!(machine_class: "skid_steer", version: "alpha-1")
    ...
    assert skid.items.all? { |i| i["photo_required"] == true && i["default_if_ambiguous"] == "round_up" }
  ...
  test "published templates are immutable; a change is a new version" do
    t = ChecklistTemplate.find_by!(machine_class: "dozer", version: "alpha-1")
    t.items = t.items.first(3)
    assert_not t.valid?
    assert_match(/immutable/, t.errors.full_messages.join)
    v2 = ChecklistTemplate.create!(machine_class: "dozer", version: "alpha-2", items: t.reload.items)
    assert_not v2.published?
    assert_equal "alpha-1", ChecklistTemplate.select_for(machine_class: "dozer", drive_type: "tracked", has_def: false).version
    v2.publish!
  ...
  test "specific variant outranks wildcard at the same version" do
    base = ChecklistTemplate.find_by!(machine_class: "excavator", version: "alpha-1")
    specific = ChecklistTemplate.create!(machine_class: "excavator", drive_type: "tracked", has_def: true, version: "alpha-1",
                                         items: base.items)
    specific.publish!
